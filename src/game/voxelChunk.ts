...
export class VoxelChunk {
	...
	getVoxel({ voxelLocalCordX, voxelLocalCordY, voxelLocalCordZ }: LocalVoxelCoordinateType) {
		if (
			voxelLocalCordX < 0 ||
			voxelLocalCordX > this.chunkWidth - 1 ||
			voxelLocalCordY < 0 ||
					...
					if (voxel && voxel !== 0) {
						for (const [faceIndex, { normal, vertexPosition, uv }] of this.faces.entries()) {
							...
							if (
								...
								!(
									voxelX + normal[0] == -1 ||
									voxelX + normal[0] == this.chunkWidth ||
									voxelY + normal[1] == -1 ||
									voxelY + normal[1] == this.chunkHeight ||
									voxelZ + normal[2] == -1 ||
									voxelZ + normal[2] == this.chunkWidth
								)
							) {
